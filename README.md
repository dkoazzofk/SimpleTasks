...
**Требования:**
    Создать N потоков (например, 4).
    Каждый поток заполняет свою часть массива случайными числами (например, если массив размера 100, а потоков 4, то каждый поток заполняет 25 элементов).
    После заполнения массива все потоки должны ожидать у барьера, пока все не закончат работу.
    Главный поток (после завершения всех потоков) вычисляет сумму элементов массива.
