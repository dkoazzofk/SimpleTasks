...
:nine: **MSGQueue** :ballot_box_with_check:
...
## Task 3: Чат между двумя процессами

**Цель:**
Отработать двустороннее взаимодействие через две разные очереди (или одну очередь с разными mtype).

**Задача:**
Создайте две программы, которые могут обмениваться сообщениями:
1. Программа 1 отправляет сообщение и ждет ответа.
2. Программа 2 принимает сообщение, выводит его, запрашивает ответ у пользователя и отправляет его обратно.

## Task 4: POSIX Message Queues вместо System V

**Задача:**
Переписать одну из предыдущих задач (например, простой чат), используя POSIX API (mq_open, mq_send, mq_receive).

## Shared Memory

## Task 1: Простой обмен сообщениями

**Задача:**
Создайте два процесса (родительский и дочерний), которые обмениваются строкой через разделяемую память.
1. Родительский процесс записывает строку в shared memory.
2. Дочерний процесс читает строку и выводит её на экран.
3. После обмена данными удалите shared memory.

**Примерный вывод:**

    Родитель записал: "Hello, child process!"  

    Дочерний процесс прочитал: "Hello, child process!"  

## Task 2: Счётчик с синхронизацией

**Задача:**
Создайте shared memory, в которой хранится целочисленный счётчик.
1. Запустите несколько процессов, каждый из которых увеличивает счётчик на 1.
2. Используйте семафоры (или мьютексы) для синхронизации доступа к shared memory.
3. После завершения всех процессов выведите итоговое значение счётчика.
...
## Task 3: Shared Memory + Очередь сообщений

**Задача:**
Создайте систему, где процессы обмениваются данными через shared memory и очереди сообщений (message queues).
1. Процесс-отправитель записывает данные в shared memory и отправляет через очередь сообщений идентификатор (shmid).
2. Процесс-получатель принимает shmid, подключается к shared memory и читает данные.
3. После прочтения получатель удаляет shared memory.

**Примерный вывод:**

    Отправитель записал в shared memory: "Important data!"  

    Получатель прочитал: "Important data!"  

    Shared memory удалена.  
